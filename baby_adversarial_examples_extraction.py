...
import pickle
from tune_weights_phase_two import load_sequence, SEQ_DIR
...
def main():
    # Load 1000 sequences
    seqs = [load_sequence(name.split(".csv")[0]) for name in os.listdir(SEQ_DIR)]
    ...
    for anchor in range(0, len(seqs)):
        ...
        with open(join("results", f"baby_adversarial_examples.pkl"), "wb") as f:
